...
void CompliantActuatorPlugin::tauElastic(const double & q1_in, const double & q2_in, const double & qL_in, double & t1_out, double & t2_out, double & tL_out, double & sigmaL_out){
    ...
    std::cout << "\033[1;33m" << warning_msg << "\033[0m\n"
              << std::endl;
};
...
void CompliantActuatorPlugin::OnUpdatePIDRefs(const common::UpdateInfo & info){
    ...
    CompliantActuatorPlugin::Publish(tauEl_1, tauEl_2, tEl_L, sigmaEl_L, qL, dqL, ref_1, ref_2 );
    CompliantActuatorPlugin::PublishMotors(mot_1.pos, mot_1.vel, mot_1.effort, mot_2.pos, mot_2.vel, mot_2.effort);
    // printForDebug(mot_1.pos, mot_2.pos, tauEl_1, mot_1.pos, mot_2.pos, qL, ref_1, ref_2);
...
void CompliantActuatorPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  {
    std::cout << "\033[1;31mA ROS node for Gazebo has not been initialized, unable to load plugin. "
              << "Load the Gazebo system plugin 'libgazebo_ros_api_plugin.so' in the gazebo_ros package)\033[0m\n"
              << std::endl;
    return;
  }
  //   n->param<std::string>("namespace", ns_name, "template");
  //   n->param("T_sample", T_sample, 0.01);
  ns_name = _sdf->GetElement("namespace")->Get<string>();
  T_sample = _sdf->GetElement("t_sample")->Get<double>();
...
//   std::cout << "\033[1;32m" << ok_msg << "\033[0m\n"
//             << std::endl;
