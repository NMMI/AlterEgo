...
qb_class::qb_class(){
	...
	node_ = new ros::NodeHandle("qb_interface_node");

	// Get param from roslaunch or yaml file
	std::string ns_name;
	ns_name = ros::this_node::getNamespace();
	node_->param(ns_name+"/eq_preset", flagCMD_type_, true);
	node_->param(ns_name+"/hand_perc", flag_HCMD_type_, false);
	node_->param(ns_name+"/current", flag_curr_type_, false);
	node_->param<double>(ns_name+"/step_time", step_time_, 0.002);
	node_->param<string>(ns_name+"/port", port, ns_name+"/dev/ttyUSB0");
	node_->param<int>(ns_name+"/baudrate", br, 2000000);
	node_->searchParam(ns_name+"/IDcubes", aux);
	node_->getParam(aux, ID_cube);

	node_->searchParam(ns_name+"/IDhands", aux);
	node_->getParam(aux, ID_hand);

	node_->param<string>(ns_name+"/unit", aux, "DEG");
	...
	if (!open(port.c_str(), br)){
		...
		std::string robot_name = getenv("ROBOT_NAME");
		std::string path;
		
		node_->param<string>("/"+robot_name+"/SystemCheckPath", path, "/catkin_ws/src/AlterEGO_v2/utils/SystemCheck.txt");
		std::string system_check = home+path;
  		system_check_file.open (system_check, std::ios_base::app);
	 	system_check_file<< "[ERROR] USB " << port << " was not open correctly."<<endl;
	...
	if (flagCMD_type_ != EQ_PRESET){
	    ...
	    for (int i = cube_chain_.size(); i--;){
	        if(!cube_chain_[i]->getPosAndPreset(&position, &pSet, meas_unit_)) {
	            cerr << "[WARNING] Unable to retrieve measurements of cube: " << cube_chain_[i]->getID() << "| riga 473"<< endl;
	            status = false;
...
bool qb_class::readCurrent() {
	...
	for (int i = cube_chain_.size(); i--;){
		if (!cube_chain_[i]->getCurrents(meas)){
			...
			cube_current_1.push_back(static_cast<int>(NAN));
			cube_current_2.push_back(static_cast<int>(NAN));
	...
	for (int i = hand_chain_.size(); i--;){		
		if (!hand_chain_[i]->getCurrents(meas)){
            ...
            hand_current.push_back(static_cast<int>(NAN));
        }else
	...
	if (flagCMD_type_ == EQ_PRESET){
	    ...
	    for (int i = cube_chain_.size(); i--;){
	    	if (!cube_chain_[i]->getPosAndCurr(position, current, meas_unit_)){
				cerr << "[WARNING] Unable to retrieve measurements of cube: " << cube_chain_[i]->getID() << "| riga 1002"<< endl;
	            status = false;	
