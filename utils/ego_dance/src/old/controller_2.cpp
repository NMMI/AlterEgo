...
//-------------------------------------------------------------------------------
//
//	Callback of the button that regulate the switch between followme/dance/teleop
//		Mode =  0			- Teleop
//				1			- Dance
//				2			- FollowMe
//
//-------------------------------------------------------------------------------
void EgoDance::callback_switch_mode (const std_msgs::Int8::ConstPtr &msg)
{
  mode = (int)msg->data;
}
...
EgoDance::EgoDance()
{
  ...
  nh.getParam("IN_topic_switch_mode", IN_topic_switch_mode); // Per andare in play
  // Topic you want to publish:

  controller = nh.advertise<geometry_msgs::TwistStamped>("wheels/segway_des_vel", 1);
  ...
  robot_state = nh.subscribe("AlteregoState",1, &EgoDance::callback_robot_state, this);
  sub_switch_mode	= nh.subscribe("/"+robot_name + IN_topic_switch_mode, 1, &EgoDance::callback_switch_mode, this);
...
//-------------------------------------------------------------------------------
//						ENABLING
//	Wait the flag on the topic switch the mode
//
//-------------------------------------------------------------------------------
void EgoDance::Enabled(){
...
void EgoDance::run()
{
  ...
  control_law.twist.linear.x = v / R_;
  control_law.twist.angular.z = omega;
...
void EgoDance::stability_check(){
  ...
  if (fabs(Q[3]) > 0.25 && stability_checked)
  {
    ...
    // azzero le velocità
    control_law.twist.linear.x = 0.0;
    control_law.twist.angular.z = 0.0;
