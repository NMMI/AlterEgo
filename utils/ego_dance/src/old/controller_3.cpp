...

{
  ...
  nh.getParam("/gain_controllers/lambda_5", lambda_5);
  nh.getParam("/gain_controllers/lambda_6", lambda_6);
  nh.getParam("/gain_controllers/k1", k1);
  ...
  time = nh.advertise<geometry_msgs::Pose>("/time", 1);
  ref = nh.advertise<geometry_msgs::Pose>("/ref", 1);
  ...
  Pm_ref = Eigen::VectorXd::Zero(2);     // global frame
  Pm_ref_old = Eigen::VectorXd::Zero(2); // global frame
  Pm_ref_l = Eigen::VectorXd::Zero(2);   // local frame
  theta_ref = 0;
  theta_ref_old = 0;
...
void ego_dance::new_target_points()
{
  ...
  double xm_i = (Pt1(0) + Pt2(0)) / 2;
  double ym_i = (Pt1(1) + Pt2(1)) / 2;
...
// 3° CONTROL:
// -----------------------------------------------------------------------------------------------
// transformation_3 would be the same function as trasformation 2, this is the reason why I use the same function
void ego_dance::transformation_2()
{
...
// void ego_dance::transformation_3()
// {

//   // 1° transformation:
//   // to calculate the angle 𝜗_refᵢ

//   double xm = (Pt1(0)+Pt2(0))/2;
//   double ym = (Pt1(1)+Pt2(1))/2;

//   Pm_ref << xm , ym;

//   std::cout << "Pm_ref = "<< Pm_ref <<  std::endl;
...
//   theta_ref = atan2(Pt1_new(1),Pt1_new(0));

//   // 2° transformation:
//   // to calculate, in local reference frame, the middle point between the robot hands

//   Eigen::Vector2d q_g (q(0),q(1));
//   Eigen::Vector2d Pm_ref_g (Pm_ref(0),Pm_ref(1));
...
//   Eigen::VectorXd Pm_ref_l_1 (Rgl.transpose()*Pm_ref_g - Rgl.transpose()*dgl);
//   Pm_ref_l << Pm_ref_l_1(0) , Pm_ref_l_1(1);
...
void ego_dance::transformation_3_new() // works with the new shifted target points
{

  // 1° transformation:
  // to calculate the angle 𝜗_refᵢ

  double xm = (Pt1_t(0) + Pt2_t(0)) / 2;
  double ym = (Pt1_t(1) + Pt2_t(1)) / 2;
  ...
  theta_ref = atan2(Pt1_new(1), Pt1_new(0)) + (0.5) * pi;
...
void ego_dance::compute_error_3()
{
  delta_l = Pm_ref_l - q_l;
  diff_angle = theta_ref - Q(2);
...
void ego_dance::compute_vel_3()
{

  geometry_msgs::Pose time_vec;
  time_vec.position.x = t;
  time_vec.position.y = t_old;
  time_vec.position.z = t - t_old;
  time.publish(time_vec);

  // to calculate the v_ref and omega_ref of the middle point between the target points (Pm_ref)
  if (flag == 0)
  {
    v_ref = 0.0;
    omega_ref = 0.0;
  ...
  double period = t - t_old;
  double vx_ref = (Pm_ref(0) - Pm_ref_old(0)) / period;
  double vy_ref = (Pm_ref(1) - Pm_ref_old(1)) / period;
  v_ref = sqrt(pow(vx_ref, 2) + pow(vy_ref, 2));
  omega_ref = (theta_ref - theta_ref_old) / (t - t_old);

  if (fabs(omega_ref) > 2)
  {
    omega_ref = ((fabs(omega_ref)) / omega_ref) * 2;
  }
  else if (fabs(omega_ref) < 0.1)
  {
    omega_ref = 0;
  }
  ...
  if (fabs(v_ref) > 3 * R_)
  {
    v_ref = ((fabs(v_ref)) / v_ref) * 3 * R_;
  }
  else if (fabs(v_ref) < 0.2 * R_)
  {
    v_ref = 0;
  }

  v_ref = (1 - 0.005) * v_ref_old + 0.005 * v_ref;
  v_ref_old = v_ref;
  omega_ref = (1 - 0.005) * omega_ref_old + 0.005 * omega_ref;
  ...
  std::cout << std::endl;
  std::cout << "Pm_ref-Pm_ref_old = " << Pm_ref - Pm_ref_old << std::endl;
  std::cout << std::endl;

  geometry_msgs::Pose ref_vec;
  ref_vec.position.x = v_ref;
  ref_vec.position.y = omega_ref;
  ref.publish(ref_vec);
}

void ego_dance::controller_3()

{
  if (fabs(delta_l(0)) <= 1e-4)
  {
    v = 0;
    omega = 0;
    std::cout << "sono nell' if " << std::endl;
  ...
  else
  {
    ...
    v = v_ref * cos(diff_angle) - lambda_5 * delta_l(0);
    omega = omega_ref - ((delta_l(1) * v_ref) / k1) - lambda_6 * sin(diff_angle);
  }
...
void ego_dance::run()
{
  ...
  // std::cout << "Pm_ref = "<< Pm_ref <<  std::endl;
  // std::cout << "Pm_ref_old = "<< Pm_ref_old <<  std::endl;
  // std::cout << "t - t_old = "<< t - t_old<<  std::endl;

  Pm_ref_old = Pm_ref;
  theta_ref_old = theta_ref;
  t_old = t;

  std::cout << "Execute Control 3" << std::endl;
  std::cout << "-----------------------------------------------------" << std::endl;
  std::cout << std::endl;
  ...
  // publish for rosbag su topic /data:
  ...
  vec2.position.x = Pm_ref(0);
  vec2.position.y = Pm_ref(1);
