...
int main(int argc, char **argv)
{
	...
	std_msgs::Float64MultiArray 	msg_act_fb;
	...
	ros::Publisher 	pub_act_fb  = n.advertise<std_msgs::Float64MultiArray>(act_fb_topic, 1);
...
//			actValue = input_act[0];
		...
		if (pub_topic_enabled && (idx%6)==0){
			msg_act_fb.data.clear();
			
			//msg_act_fb.data = actValue;
			
			for (int i = 0; i < 6; i++){
				msg_act_fb.data.push_back(actValue[i]);
			}
